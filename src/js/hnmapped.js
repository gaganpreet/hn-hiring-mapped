...
var showData = function() {
    ...
    $.each(all_posts, function(index, post) {
        ...
        post_html = highlightSearchTerms(post['full_html'], filters['text_filter']);
    ...
    status_html = Mustache.render("<p>Displaying {{ count }} posts</p>", {"count": count});
...
var doesMatchFilters = function(item) {
    ...
    if (filters['location_filter']) {
        ...
        var re = new RegExp(filters['location_filter'], 'i');
    ...
    if (filters['text_filter']) {
        var text = item['full_text'];
        var re = new RegExp(filters['text_filter'], 'i');
        if (!re.exec(text)) {
            match = false;
        }
    }
...
var showMonthList = function() {
    ...
    $.each(available_data.reverse(), function(index, value) {
        ...
        var year = parseInt(valueArray[0]);
        var month = parseInt(valueArray[1]) - 1; 
